...
| 档 | 状态 | 结果 |
|---|---|---|
| **P0** 承接语提示词 | ✅ **已做，已验收** | **22%（9/41）** < 目标 50% → **不达标** |
| **P1** 填充音改触发时机 | ✅ **已做，但改了方向** | 见下 |
| **P2** 文案从「无内容」改「有内容」 | ⛔ **方向被调研否掉** | 见下 |
| （新）**异步工具** | ✅ 已做 | `WORKORDER-ASYNC-01.md`：回合从 23s 提前到 4.4s 收尾 |

**P1/P2 的方向被 2026-09-20 的行业调研推翻**（原文写在 `WORKORDER-LEADIN-01.md` §1）：
原计划是「把通用填充音**加长**」，但证据指向反方向 —— 通用 filler 是**收益最弱、
副作用最明确**的一档（OpenAI：用不好反而**增加**感知延迟；CUI'25：改善感知但不改善
参与感；ConvFill：Naturalness 偏低、用户说"显得重复"；Jeong：用简单 filler 的 agent
**被认为更不聪明**）。**上下文相关**的 filler 才明显更好。

→ 实际做的是：**按工具名选句子**（搜东西→「我搜一下」/ 开浏览器→「我开个页面看看」/
翻笔记→「我翻下笔记」/ 跑命令→「我跑一下」），覆盖真机 98% 的工具调用。

**本机实测的覆盖率账**（工具轮，32 轮）：

| 阶段 | p50 | p90 | max |
|---|---|---|---|
| 工具前（用户→首个工具）| 2.9s | 7.5s | 23.9s |
| 工具后（末个工具→首句）| 4.1s | 7.1s | 9.6s |
| **合计** | **14.3s** | **49.3s** | **113.8s** |

⇒ 现有 0.96s 的填充音对三段的覆盖率都是 **0%**。要覆盖 80% 需要 4.9s / 5.6s / 28.1s
—— 这就是为什么最后走的是**异步工具**（工具后台跑，回合立刻收尾）而不是把填充音拉长。
