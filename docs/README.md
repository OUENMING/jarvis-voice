...
| **`ROOTCAUSE-BARGEIN-ASR-20260920.md`** | ⭐⭐ **打断时识别差的根因定案** —— 🟢 **AEC3 双讲时削掉近端高频**（原始麦克风 4–8k 13–17.5% → 过 AEC 只剩 0.1–0.3%）；物理/管道/模型三条假设全部排除；含**真机前后对照音频**、离线控制实验、**压制强度取决于「能否用 far 解释近端」**（我们的 150ms 是估计值）、以及四条修复方向 |
